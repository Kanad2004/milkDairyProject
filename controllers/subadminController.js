...
const addSubAdmin = async (req, res) => {
  ...
  if (!profilePicture) {
    res.json(new ApiError(400, "Cover Img is missing"));
    return;
  }
  ...
  if (subAdminExists) {
    // throw new ApiError(400, "SubAdmin already exists!");
    // res.json(new ApiError(400, "SubAdmin already exists!"));
  ...
  if (!existingBranch) {
    // throw new ApiError(400, "Branch does not exist!");
  ...
  if (!profilePictureImg.url) {
    ...
    return res
      .status(400)
      .json({ error: "Error while uploading ProfileImage on cloudinary" });
  }

...
const displayAllProducts = asyncHandler(async (req,res) => {
    ...
    if(!products){
        ...
        return res.status(200).send(new ApiResponse(200 , products , "All products got successfully . .."))
    }
});
...
const displayProductByName = asyncHandler(async (req , res) => {
    ...
    if(!products){
        ...
        return res.status(200).send(200 , products , "fetched all the products with the name ")
    }
});
...
const updateProduct = asyncHandler(async (req,res) => {
    ...
    return res.status(200).send(200 , updateProduct , "Product updated successfully")
});
...
const addProduct = asyncHandler(async (req,res) => {
    const {productName , productPrice , productDescription , productMassPercentage} = req.body ; 
    
    if([productName , productPrice , productDescription , productMassPercentage].some((field) => field?.trim === "")){
...
const deleteProductById = asyncHandler(async (req , res) => {
  try {
    ...
    return res.status(200).send(new ApiResponse(200 , {} , "Product deleted successfully"))
  } catch (error) {
...
export {displayAllProducts , displayProductByName , updateProduct , addProduct , updateProductImg ,deleteProductById} 